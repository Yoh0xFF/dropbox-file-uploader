...
public class App {
  ...
  public static void main(String[] args) {
    String dbxAccessToken = System.getenv("DBX_ACCESS_TOKEN");
    if (isBlank(dbxAccessToken)) {
      logger.error("Please set DBX_ACCESS_TOKEN environment variable");
      return;
    }

    String dbxClientName = System.getenv("DBX_CLIENT_NAME");
    ...
    DbxRequestConfig dbcRequestConfig = DbxRequestConfig.newBuilder(dbxClientName).withUserLocale("en_US").build();
    ...
    String curMonth = curDate.getMonthValue() < 10 ? "0" + curDate.getMonthValue() : "" + curDate.getMonthValue();
